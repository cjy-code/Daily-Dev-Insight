...
> **Date**: 2026-08-05
> **Status**: **1차+2차 사이클 완료 + 처리흐름도 전수화 (v0.5)** — 17개 compact card(전부 Mermaid 처리흐름도 포함) + 승격 후보 3개(Unit 2/8/13) 정밀 문서화, Codex 교차검증 4회 반영, F-18 보안 수정 완료. 잔여 findings 26건은 백로그로 유지
...
| 항목 | 저장 위치 | 내용 |
|---|---|---|
| Compact Card | `docs/02-design/features/{unit}.md` (파일 1개) | ① 목적(1~2문장) ② 관련 파일(컨트롤러/서비스/엔티티/템플릿) ③ 진입 엔드포인트 ④ 핵심 호출 흐름(bullet, 5~10줄) + **처리 흐름도(Mermaid, 2026-08-06 전체 소급 추가)** ⑤ 데이터/외부 연동 ⑥ 인증·트랜잭션·캐시 처리 방식 ⑦ 화면 요약(있는 경우, 주요 상태만) ⑧ 패턴 특이사항(다른 unit과 다르게 구현된 부분 — Codex 2차 검증에서 실제 편차 확인됨) ⑨ 알아둘 점/리스크 |
...
- [x] 17개 unit 전체에 compact card 존재 (1차 사이클 완료, 2026-08-05)
- [x] §2.4 1차 승격 후보(Unit 2, 8, 13)는 정밀 문서로 승격 + Codex 교차검증 완료 (2차 사이클, 2026-08-06)
- [x] `docs/MVP_SCOPE.md` §4 문서 부채 목록에서 완료 항목 갱신 (2026-08-06)

### 4.2 Quality Criteria

- [x] 정밀 문서로 승격된 unit(2/8/13)은 Codex 검증에서 발견된 불일치를 전량 정정해 현재 시점 기준 코드와 일치 — 단, Unit 13처럼 최초 판정 자체가 뒤집힌 사례가 있어 "처음부터 100%"가 아니라 "검증-정정 후 100%"였음을 기록으로 남김(각 Analysis 문서 참조)
- [x] compact card 단계 unit(1,3~7,9~12,14~17)은 §2.1의 9개 필수 항목 포함 확인
...
### 8.5 Compact Card 처리 흐름도 소급 추가 (2026-08-06)

2차 사이클 정밀화 대상(Unit 2/8/13)에만 있던 Mermaid 처리 흐름도를, 사용자 요청으로 **나머지 14개 compact card unit(1,3~7,9~12,14~17)에도 전부 소급 추가**했다. compact card 템플릿(§2.1)의 ④ 항목에 "처리 흐름도"를 정식 반영. 화면 설계 문서(`docs/02-design/screens/`)에는 흐름도를 중복 배치하지 않고 기능 설계 문서로 링크하는 기존 원칙은 유지.
...
| 0.5 | 2026-08-06 | 사용자 요청으로 나머지 14개 compact card unit에 처리 흐름도(Mermaid) 소급 추가(§8.5). §2.1 compact card 템플릿 정의 갱신 | Claude (사용자 요청) |
