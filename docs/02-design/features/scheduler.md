...
```mermaid
flowchart TD
    A["@Scheduled(매분) 실행"] --> B["getOrCreateSchedule()"]
    B --> C{"enabled=true?"}
    C -->|아니오| D["skip"]
    C -->|예| E["cron 표현식 파싱\n(lastExecutedAt 또는 now-1분 기준)"]
    E --> F{"다음 실행시각 <= now?"}
    F -->|아니오| D
    F -->|예| G["실제 작업 실행\n(크롤링: TechNewsCrawlingService\n생성: DailyKnowledgeGenerationService)"]
    G --> H{"성공?"}
    H -->|예| I["markExecuted(now): lastExecutedAt 갱신"]
    H -->|아니오| J["갱신 안 함 → 다음 폴링에서 재시도"]
```
